...
function App() {
  ...
  const firstPlayRef = useRef(true);     // flag to avoid playing sound first time
  ...
  useEffect(() => {
    ...
    if (isRunning) {
      interval = setInterval(() => {
        setCountDown(prev => {
          if (prev <= 0) {
            // Play the sound only after user interaction and if it's not the first play
            if (!firstPlayRef.current && userInteracted) {
              audioRef.current.play().catch(err => console.warn("Audio play failed:", err));
            }

            firstPlayRef.current = false;  // after first session
